...
def solve_diffusion(t, x, kappa, u_I, boundary_conditions=[('Dirichlet', lambda t : 0), ('Dirichlet', lambda t : 0)], 
    ...
    match discretization:
        ...
        case 'Backward Euler':
            u = backw_eul_solve_diffusion(u,t,x,kappa_t,kappa_t_dep,boundary_conditions,bounds,rhs_func,
                kappa_args,kappa_kwargs,func_args,func_kwargs)
...
def test_simple_heat_eq_backw_euler():
...
start = time.time()
maybe = test_simple_heat_eq_backw_euler()
end = time.time()
if maybe:
    print(end-start)
    print('yay')
